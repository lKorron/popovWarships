...
class Grid:
    ...
    def move_ship_randomly(self, ship):
        ...
        if directions:
            ship.direction = random.choice(directions)
            self.move_ship(ship)
...
class Ship:
    def __init__(self, color, speed=1, vision_range=3, attack_range=1, health=100, power=10, accuracy=0.7, attack_delay=30):
        ...
        self.direction = "up"  # Направление корабля по умолчанию
            ...
            if ships_in_range:
                for target in ships_in_range:
                    if target.ship_type != ship.ship_type and ship.should_move_towards(target):
                        if ship.direction == "up" and target.pos_y < ship.pos_y:
                            grid.move_ship(ship)
                        elif ship.direction == "down" and target.pos_y > ship.pos_y:
                            grid.move_ship(ship)
                        elif ship.direction == "left" and target.pos_x < ship.pos_x:
                            grid.move_ship(ship)
                        elif ship.direction == "right" and target.pos_x > ship.pos_x:
                            grid.move_ship(ship)
                        else:
                            if target.pos_x != ship.pos_x:
                                if (target.pos_x < ship.pos_x and ship.direction != "left") or (target.pos_x > ship.pos_x and ship.direction != "right"):
                                    ship.turn_left() if random.random() < 0.5 else ship.turn_right()
                            elif target.pos_y != ship.pos_y:
                                if (target.pos_y < ship.pos_y and ship.direction != "up") or (target.pos_y > ship.pos_y and ship.direction != "down"):
                                    ship.turn_left() if random.random() < 0.5 else ship.turn_right()
                        break
            else:
